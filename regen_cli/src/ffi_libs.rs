...
use regen_core::{env::{self, CellIdentifier, Env, define_global}, ffi_libs::RegenString, hotload, parse::{self, Expr}, symbols::{Symbol, to_symbol}, types, types::c_function_type};
use std::fs;
use std::path::Path;
use std::ffi::CString;
...
pub extern "C" fn include(env : Env, file_name : &RegenString) {
  ...
  let id = CellIdentifier::def(env::new_namespace(&[module_namespace]), path_symbol);
  let uid = id.uid(env);
  if env::get_cell_value(env, uid).is_none() {
    ...
    hotload::hotload_module(env, file_name, &code);
